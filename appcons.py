    ...
    for f_path in files:
        try:
            ...
            if "Account" not in temp_df.columns:
                temp_df["Account"] = sl.extract_source_from_filename(os.path.basename(f_path))
            ...
            if "Counterparty" not in temp_df.columns:
                if "From" in temp_df.columns and "To" in temp_df.columns:
                    def get_cp_fast(row):
                        f_addr = sl.resolve_raw_addr(str(row.get("From", "")))
                        acc_id = sl.resolve_raw_addr(str(row.get("Account", "")))
            ...
            # UNIFICATION CASE
            temp_df = sl.standardize_df_addresses(temp_df)
...
with tab_list:
    ...
    if "Account" in df_disp.columns:
        df_disp["Account"] = df_disp["Account"].apply(sl.resolve_raw_addr).apply(sl.resolve_owner_display)
    ...
    if df.empty:
        ...
        if df_at_date.empty:
            ...
            if "vgp_df" in st.session_state:
                ...
                res_df_disp["Account"] = res_df_disp["Account"].apply(sl.resolve_owner_display)
                ...
                by_acc["Account"] = by_acc["Account"].apply(sl.resolve_owner_display)
                ...
                def generate_vgp_pdf(data_df, date_str, total_val):
                    ...
                    for _, row in data_df.iterrows():
                        disp_acc = sl.resolve_owner_display(row["Account"])
