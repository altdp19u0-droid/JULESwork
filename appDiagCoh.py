    ...
    if not df_j.empty:
        for _, r in df_j.iterrows():
            ...
            if r.get("Category") == "Transfert Interne":
                cp_raw = sl.resolve_raw_addr(r["Counterparty"])
...
with st.sidebar:
    ...
    if target_year != st.session_state.get("last_diag_year"):
        sl.clean_session_state(preserve_keys=["last_diag_year", "_hub_diag_year"])
        st.session_state.last_diag_year = target_year
        st.cache_data.clear()
        st.rerun()
...
if history.empty:
    ...
    with tab_tracer:
        ...
        sel_acc = st.selectbox("Choisir un compte", all_accs, format_func=sl.resolve_owner_display)
