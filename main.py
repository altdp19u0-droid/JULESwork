...
# --- Configuration ---
st.set_page_config(page_title="Jules Crypto Tracker Ultimate", layout="wide")
...
def get_price(asset, date_obj):
    try:
        # Mapping enrichi pour inclure les assets demandés
        cg_map = {
            "ETH": "ethereum", "BNB": "binancecoin", "POL": "polygon-ecosystem-token",
            "USDT": "tether", "USDC": "usd-coin", "EURA": "eura", "STEUR": "stasis-euro",
            "8LND": "8lnd", "BTC": "bitcoin", "WBTC": "wrapped-bitcoin", "DAI": "dai"
        }
        asset_id = cg_map.get(asset.upper())
        if not asset_id: asset_id = asset.lower()
...
def fetch_data(address, api_key, network):
    ...
    # Indicateurs de récolte détaillés
    results_summary = st.expander(f"📊 Rapport détaillé de récolte : {network}", expanded=True)

    count_free = 0
    count_api = {"Native": 0, "Tokens": 0, "Internal": 0}
    ...
    try:
        if network == "Ethereum":
            ...
            if isinstance(data, list):
                ...
                for t in data:
                    txs.append({
                        'Source': 'Libre (Ethplorer)', 'ID': t['hash'],
                        'Date': datetime.fromtimestamp(int(t['timestamp'])),
                        'Account': address, 'Asset': 'ETH',
                        'Type': 'Mvt', 'Amount': float(t.get('value', 0)),
                        'Fee': 0.0, 'Counterparty': t['from'],
                        'Network': network
                    })
        elif "blockscout" in cfg['free_api']:
            # Native + Tokens via Blockscout API v2
                    ...
                    try:
                        ...
                        if "items" in data:
                            ...
                            for t in items:
                                ...
                                txs.append({
                                    'Source': f'Libre ({sub})', 'ID': t['hash'],
                                    'Date': dt, 'Account': address, 'Asset': asset,
                                    'Type': 'Mvt', 'Amount': amount, 'Fee': fee,
                            ...
                            if data.get("next_page_params"):
                                ...
                                url = f"{cfg['free_api']}/addresses/{address}/{sub}?{query}"
                            else: break
                        else: break
                    except: break
    except Exception as e:
        results_summary.write(f"ℹ️ Info Libre {network}: {e}")

    # 2. VOIE API (Clones Etherscan)
    if api_key:
        try:
            # txlist (Native), tokentx (ERC20), txlistinternal (Internal)
            for action in ["txlist", "tokentx", "txlistinternal"]:
                url = f"https://{cfg['host']}/api?module=account&action={action}&address={address}&startblock=0&endblock=99999999&page=1&offset=10000&sort=desc&apikey={api_key}"
                res = requests.get(url, timeout=15).json()
                if str(res.get("status")) == "1":
                    results = res.get("result", [])
                    if action == "txlist": count_api["Native"] += len(results)
                    elif action == "tokentx": count_api["Tokens"] += len(results)
                    else: count_api["Internal"] += len(results)
                    ...
                    results_summary.error(f"❌ Clé API {network} ({action}) rejetée : {res.get('result')}")
                elif res.get("message") != "No transactions found":
                    results_summary.warning(f"⚠️ API {network} ({action}) : {res.get('message')}")
        except Exception as e:
            results_summary.error(f"Erreur Voie API {network}: {e}")

    results_summary.write(f"✅ **Voie Libre :** {count_free} transactions")
    results_summary.write(f"✅ **Voie API :** {count_api['Native']} Natives, {count_api['Tokens']} Tokens, {count_api['Internal']} Internes")

    df = pd.DataFrame(txs)
    if not df.empty:
        # Fusion critique : on garde toutes les lignes d'assets différents pour une même ID
        df = df.drop_duplicates(subset=['ID', 'Asset', 'Amount'])

        # Conversion EUR optimisée
        with st.spinner(f"Conversion EUR historique ({len(df)} lignes)..."):
            ...
            if total_p > 0:
                pbar = st.progress(0, text="Calcul des cours...")
                for i, (_, row) in enumerate(unique_combos.iterrows()):
                    ...
                    pbar.progress((i + 1) / total_p, text=f"Calcul des cours ({i+1}/{total_p})...")
                pbar.empty()
        ...
        def detect_cat(row):
            ...
            if any(x in cp for x in ['swap', 'pool', 'staking', 'lending', 'uniswap', 'aave', 'pancake']): return 'Position DeFi'
            if any(x in asset for x in ['steth', 'steur', 'lnd', 'savings']): return 'Position DeFi (Asset)'
...
if menu == "Dashboard":
    st.header("📋 Tableau de bord des comptes propriétaires")
    col_d1, col_d2 = st.columns([3, 1])
    with col_d1:
        with st.form("add_account"):
            c1, c2 = st.columns(2)
            addr_in = c1.text_input("Adresse (0x...)"); net_in = c2.selectbox("Réseau", list(NETWORKS_CFG.keys()))
            key_in = st.text_input(f"Clé API {net_in} (Recommandée pour Tokens)", type="password")
            if st.form_submit_button("Lancer l'exploration massive"):
                if addr_in:
                    ...
                    st.success(f"Exploration terminée ! {len(df)} transactions récoltées.")
    with col_d2:
    ...
    if not st.session_state.transactions.empty:
        ...
        f_cat = c3.multiselect("Catégories", df['Category'].unique())
        ...
        edited = st.data_editor(df, use_container_width=True, column_config={
            ...
            "Fee": st.column_config.NumberColumn("Frais (Native)", format="%.8f")
        })
        for s in edited[edited['Is_Spam']==True]['Counterparty'].unique():
            if s not in st.session_state.spam_addresses:
                st.session_state.spam_addresses.add(s); st.rerun()
    else: st.info("Aucune donnée.")

elif menu == "Fiat & Fiscalité":
    st.header("⚖️ Fiat & Fiscalité France")
    st.subheader("💶 Gestion des comptes Fiat")
    ...
    col_m1.metric("Prix d'acquisition Net", f"{investi - retire:,.2f} €")
    col_m2.metric("Valeur Portefeuille (Historique)", f"{port_val:,.2f} €")

elif menu == "Export":
    st.header("📥 Exports")
    v = st.session_state.transactions[~st.session_state.transactions['Counterparty'].isin(st.session_state.spam_addresses)]
    st.download_button("Exporter Valides", v.to_csv(index=False).encode('utf-8'), "mon_historique.csv")

st.divider(); st.caption("Jules AI Ultimate Pro - Harvest Fix - Windows 11")
