    ...
    for action, label in endpoints:
        ...
        try:
            ...
            if res_status == "0":
                ...
                return items, str(res_result) # Real error
...
def fetch_blockscout_v2(api_v2, addr, max_items, year, endpoint):
    ...
    for page in range(100):
        ...
        for item in data["items"]:
            ...
            # Do NOT exit early, there might be mixed timestamps in pages
...
# --- Harvest Loop ---
harvest_btn = st.button("🚀 Lancer la Récolte Totale", width='stretch')
...
if harvest_btn:
    if not active_addr or not Web3.is_address(active_addr): st.error("❌ Adresse invalide.")
            ...
            try:
                ...
                for t in raw_bs_native:
                    ...
                    global_raw_txs.append({
                        "Date": dt.isoformat(), "Chain": chain, "Tx_Hash": tx_h, "Type": "Native", "Method": t.get("method", ""), "Account": addr_c,
                ...
                for t in raw_bs_toks:
                    ...
                    global_raw_txs.append({
                        ...
                        "Fee_Asset": "", "Fee_Amount": 0.0, "Source_Way": "Way_1", "Audit_Status": "RAW", "Fee_Audit_Alert": "", "Source_Exchange_Rate": 0.0
                    })
                chains_status[chain]["bs"] = True
            except: chains_status[chain]["bs"] = False

            # 2. Etherscan V2 (Way 2)
            if v2_api_key:
        ...
        if not df_m.empty:
            ...
            # Add occurrence count to distinguish identical legs in one Tx
            df_m["_occ"] = df_m.groupby(["Tx_Hash", "Asset", "Chain", "_amt_round", "From", "To", "Source_Way"]).cumcount()
            ...
            st.session_state.account_data_registry[addr_c] = {"transactions": st.session_state.transactions.copy(), "status": chains_status}
        st.rerun()
...
if display_registry := st.session_state.account_data_registry:
    for addr, data in display_registry.items():
        with st.expander(f"👤 Compte : {sl.resolve_owner_display(addr)}", expanded=True):
            ...
            if not df_t.empty:
                # Identification Natives (Base Assets)
                mask_tok = (df_t["Type"].str.contains("Token")) | (~df_t["Asset"].isin(["ETH", "POL", "BNB", "AVAX"]))
                st.write(f"📝 **Transactions Natives** ({len(df_t[~mask_tok])})")
                st.dataframe(df_t[~mask_tok], width='stretch')
                st.write(f"🪙 **Transferts de Tokens** ({len(df_t[mask_tok])})")
                st.dataframe(df_t[mask_tok], width='stretch')
                if st.button(f"💾 Sanctuariser {addr[:10]}...", key=f"save_{addr}"):
                    ...
                    df_t.to_csv(os.path.join(y_dir, f"raw_transactions_consolidated_{addr}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"), index=False, encoding="utf-8-sig")
                    st.success("Enregistré.")
            else: st.warning("Vide.")

sl.show_status(); st.sidebar.caption("Harvest V2 Multi-Domain v7.3")
