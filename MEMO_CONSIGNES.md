...
2. **appPropri (Dashboard) :**
    ...
    - **Anti-Inflation VGP :** Le mirroring des positions protocoles n'est appliqué que si la position n'est pas déjà présente comme compte actif dans l'historique, évitant les doubles comptages.
    - **Unité de Calcul :** Le prix d'acquisition total (A) est calculé exclusivement à partir du journal CLEAN qualifié (Achat asset EUR) pour assurer une cohérence parfaite entre les vues.
