...
2. **appPropri (Dashboard) :**
    ...
    - **Correction Stablecoins Fiat-Sourced :** Pour éviter la redondance entre le prix d'acquisition (A) et la VGP, le système déduit de la VGP brute la quantité de stablecoins (EURA/EURC) achetés via fiat et encore présents en portefeuille. Dès que ces stables sont échangés (Swap/Vente), ils sont considérés comme consommés.
    - **Continuité EOY :** Le calcul du snapshot au 31/12 utilise par défaut l'inventaire sanctuarisé de l'année précédente (`inventory_EOY_N-1.csv`) comme point de départ ("Report") pour garantir la continuité des soldes. Un mode "Recalculer tout l'historique" permet de passer outre ce report.
    - **Forçage Manuel de la VGP :** L'App 2 VGP permet de définir manuellement une valeur de VGP nette à une date donnée (ex: 31/12) via un fichier persistant `vgp_overrides.json`. Cette valeur forcée prévaut sur le calcul automatique dans tous les rapports fiscaux et tableaux de bord.
